...
openssl pkey -in ca-key.pem -out ca-key-decrypted.pem -passin pass:password
}

de()
{
rm ca-key.pem ca.pem ca.srl cert.csr cert-key.pem cert.pem extfile.cnf ca-key-decrypted.pem &>/dev/null
if [ "$?" == "0" ];then
